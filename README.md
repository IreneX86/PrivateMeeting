...
- `VITE_TURN_URLS`: optional comma-separated `turn:`/`turns:` URLs. The parser trims whitespace and creates one `RTCIceServer` whose `urls` value is the complete URL array, with the configured username and credential. This lets you supply UDP, TCP, port 80/443, and TLS endpoints from one TURN account. The previous singular `VITE_TURN_URL` remains a fallback only when `VITE_TURN_URLS` is absent; migrate existing settings to the plural name. An explicitly empty `VITE_TURN_URLS` disables TURN even if the legacy variable is present.
...
4. In **Settings → Secrets and variables → Actions → Variables**, create `VITE_SIGNALING_URL` with your `wss://HOST/signal`. Optionally create `VITE_STUN_URLS`, `VITE_TURN_URLS`, and `VITE_TURN_USERNAME`. Put the provider's comma-separated browser TURN endpoints in `VITE_TURN_URLS`; do not include a real credential in repository files. If using a static TURN client credential, store `VITE_TURN_CREDENTIAL` as an Actions secret to avoid printing it in workflow setup—but understand that the credential is still visible to every user in the delivered browser bundle.
...
STUN helps discover reachable network candidates; it does not forward media. Some NATs, firewalls, VPNs and corporate networks prevent direct connectivity. TURN supplies a relay route and consumes relay bandwidth; configure it for reliable public use. `VITE_TURN_URLS` accepts a comma-separated set of the provider's `turn:` and `turns:` endpoints, which are passed to the browser as one URL array with `VITE_TURN_USERNAME` and `VITE_TURN_CREDENTIAL`. Without TURN, a failed direct connection is expected on some networks, and the UI offers a rejoin with an explanation.
...
- 43 unit/integration tests passed, including room/protocol boundaries, source and byte limits, proxy spoofing, graceful/bounded shutdown and the production SIGTERM entrypoint, early and duplicate ICE, offered-transceiver reuse, late socket callbacks, and screen-share lifecycle races.
